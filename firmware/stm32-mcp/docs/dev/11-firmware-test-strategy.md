...
| --- | --- | --- |
| Pure firmware logic | `rev_safety_l8.c`, `tof_frame_codec.c`, `tof_l8_config.c`, `ble_tof_policy.c`, `tof_l8_topology.c` | Host unit tests and coverage |
...
| `test_tof_l8_topology` | Future front/rear sensors use safe buses, addresses, and control pins |
...
| --- | --- |
| SATEL unpowered or unplugged | UART logs `no sensor addr=0x52`; FE63 reports error state after retry |
...
1. Rear sensor alone on I2C3: A5/A4, A1 `LPn`, A2 `GPIO1`.
2. Front sensor alone on I2C1: D15/D14, A0 `LPn`, A3 `GPIO1`, with PC6 holding
   the on-board VL53L0X in reset.
3. Both sensors wired with shared 5V, GND, and `SPI_I2C_N=GND`, but separate
   SCL/SDA buses and separate `LPn` lines.
4. Both sensors ranging continuously at conservative rates.
5. Safety policy chooses the front sensor while moving forward and the rear
   sensor while reversing, while both frame streams remain alive.

Expected two-sensor logs once firmware support is added:

```text
VL53L8 rear stream start bus=I2C3 addr=0x52 layout=4 zones=16
VL53L8 front stream start bus=I2C1 addr=0x52 layout=4 zones=16
VL53L8 rear frame seq=...
VL53L8 front frame seq=...
```

If both sensors ever share one I2C bus, the boot sequence must hold both `LPn`
lines low and wake/address one sensor at a time. The preferred design avoids
that fragile address-assignment sequence by putting front and rear sensors on
separate buses.
