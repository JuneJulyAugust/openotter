...
- iOS STM32 discovery must not treat a remembered CoreBluetooth UUID as identity proof after reset. The app may use the remembered UUID only with fresh OpenOtter evidence from the latest advertisement name or FE40/FE60 service UUID; scan-timeout blind remembered reconnects stay disabled.
